...
import { filterRestoreBackupsByTarget } from '../../utils/restoreJob.utils'
...
export function LaunchRestoreModal({
  ...
  const formatBackupOptionLabel = (backup: RestoreBackupOption) => {
    const dateLabel = new Intl.DateTimeFormat(i18n.language, {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    }).format(new Date(backup.date))

    return backup.name
      ? t('common.backupFromWithName', { date: dateLabel, name: backup.name })
      : t('common.backupFrom', { date: dateLabel })
  }
  ...
  useEffect(() => {
    ...
    setName(t('pages.imports.modal.defaultName'))
  ...
  }, [isOpen, backups, targets, onClose, t])
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {t('pages.imports.modal.title')}
        </h2>

        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
          {t('pages.imports.modal.description')}
        </p>
        ...
        <div className="mt-4 space-y-4">
          <div>
            ...
            >
              {t('pages.imports.modal.restoreNameLabel')}
            </label>
            <input
              ref={nameInputRef}
              id="restore-name"
              ...
              placeholder={t('pages.imports.modal.restoreNamePlaceholder')}
          ...
          <div>
            ...
            >
              {backupsForTarget.length === 0 ? (
                <option value="">{t('pages.imports.modal.noBackupForTarget')}</option>
              ) : (
                backupsForTarget.map((backup) => (
                  <option key={backup.id} value={backup.id}>
                    {formatBackupOptionLabel(backup)}
                  </option>
                ))
              )}
            </select>
            {backupsForTarget.length === 0 && (
              <p className="mt-1 text-xs text-amber-600 dark:text-amber-400">
                {t('pages.imports.modal.noBackupHint')}
              </p>
            )}
          </div>
        </div>
