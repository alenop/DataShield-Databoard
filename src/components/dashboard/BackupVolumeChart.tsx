...
function CustomTooltip({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 shadow-md dark:border-slate-600 dark:bg-slate-800">
      ...
      <p className="text-sm font-semibold text-slate-900 dark:text-white">{point.volumeGb.toFixed(1)} Go</p>
    </div>
  )
}
  ...
  return (
    <div>
      <div className="mb-4 flex items-baseline justify-between">
        <p className="text-sm text-slate-500 dark:text-slate-400">7 derniers jours</p>
        <p className="text-lg font-semibold text-slate-900 tabular-nums dark:text-white">
          {totalVolume.toFixed(1)} Go <span className="text-sm font-normal text-slate-500 dark:text-slate-400">total</span>
        </p>
      </div>
