...
import { userStatusLabels } from '../../types/user.types'
...
export function UserActionsCell({
  ...
  if (!permissions.canAssignRole && !permissions.canToggleStatus) {
    return (
      <span className="text-xs text-slate-400 dark:text-slate-500" title="Autorisation insuffisante">
        Non modifiable
      </span>
    )
  }
    ...
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
      ...
      {permissions.canToggleStatus && (
        <button
          ...
          aria-label={`${user.status === 'active' ? 'Désactiver' : 'Activer'} ${user.name}`}
        ...
        >
          ...
          />
          <span className="sr-only">{userStatusLabels[user.status]}</span>
        </button>
      )}
    </div>
  )
