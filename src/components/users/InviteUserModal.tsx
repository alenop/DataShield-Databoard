...
import { USER_ROLES, userRoleLabels, type UserRole } from '../../types/user.types'

interface InviteUserModalProps {
  isOpen: boolean
  onClose: () => void
  onInvite: (email: string, role: UserRole) => string | null
}

export function InviteUserModal({ isOpen, onClose, onInvite }: InviteUserModalProps) {
  const [email, setEmail] = useState('')
  const [role, setRole] = useState<UserRole>('backup_operator')
  ...
  const handleSubmit = (event: FormEvent) => {
    ...
    const validationError = onInvite(email, role)
    if (validationError) {
